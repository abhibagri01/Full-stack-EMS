...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if(!attendance?.checkOut){
        ...
        //After 10 hours , mark att as checked out with status "LATE"
  ...
  { id: "attendance-reminder-corn" , triggers: [{ event: "0 0 6 * * *"}]}, //06:00 UTC = 11:30 AM IST
     async ({ step}) => {
        ...
        const activeEmployees = await step.run("get-active-employees" , async ()=>{
            ...
            return employees.map((e)=>({_id: e._id.toString(), firstName:e.firstName,
                lastName: e.lastName, email: e.email, department: e.department}))
        } )
