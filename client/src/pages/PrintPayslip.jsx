...
const PrintPayslip = () => {
    ...
    useEffect(()=> {
        api.get(`/payslips/${id}`).then((res)=> setPayslip(res.data)).catch((error)=> {
            console.error("FETCH PAYSLIP ERROR:", error);
            toast.error(error?.response?.data?.error || "Failed to fetch payslip data");
    ...
    return (
        <div className='max-w-2xl mx-auto p-8 bg-white animate-fade-in'>
          <div className='text-center border-b border-slate-200 pb-6 mb-8'>
            <h1 className='text-2xl font-bold text-shadow-amber-900 tracking-tight'>PAYSLIP</h1>
            <p className='text-shadow-amber-500 text-sm mt-1'>{format(new Date(payslip.year , payslip.month - 1), "MMMM yyyy")}</p>
          </div>
